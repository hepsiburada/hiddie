...
test('use no function', t => {
  ...
  const instance = hiddie(function (err, a, b) {
    t.error(err)
    t.equal(a, req)
    t.equal(b, res)
  })
...
test('should keep the context', t => {
  ...
  const instance = hiddie(function (err, a, b, ctx) {
    t.error(err)
    t.equal(a, req)
    t.equal(b, res)
    t.ok(ctx.key)
...
test('should add `originalUrl` property to req', t => {
  ...
  const instance = hiddie(function (err) {
    t.error(err)
  })
  const req = {
    url: '/test'
...
test('basic serve static', t => {
  const instance = hiddie(function () {
...
test('should match all chain', t => {
  t.plan(2)
  const instance = hiddie(function (err, req, res) {
    t.error(err)
    t.deepEqual(req, {
      url: '/inner/in/depth',
...
test('should match the same slashed path with paths', t => {
  t.plan(3)
  const instance = hiddie(function (err, req, res) {
    t.error(err)
    t.deepEqual(req, {
      url: '/path/test',
      originalUrl: '/path/test',
      params: {
        p: 'test'
      }
    })
  ...
  instance.use('/path/:p', function (req, res, next) {
    t.pass('function called')
    next()
  })
